...
bool VarToStr (VARIANT pvar, char *buffer)
{
	...
	switch (pvar.vt & ~VT_ARRAY)
	{
        case VT_BOOL:
		case VT_I1:
			sprintf_s(buffer, 100, "%d", pvar.iVal); break;
			// RAFAEL sprintf(buffer, "%06d",    pvar.iVal);	break;
		case VT_I2:
			sprintf_s(buffer, 100, "%d", pvar.intVal); break;
			// RAFAEL sprintf(buffer, "%06d",    pvar.intVal);	break;
		case VT_I4:
			sprintf_s(buffer, 100, "%ld",   pvar.intVal);	break;
			//RAFAEL sprintf(buffer, "%06d",   pvar.intVal);	break;
		case VT_UI1:
			//sprintf(buffer, "%u",    pvar.uiVal);	break;
			sprintf_s(buffer, 100, "%06u",    pvar.uiVal);	break;
		case VT_UI2:
			sprintf_s(buffer, 100, "%u",    pvar.ulVal);	break;
			//RAFAEL sprintf(buffer, "%06u",    pvar.ulVal);	break;
		case VT_UI4:
			sprintf_s(buffer, 100, "%lu",   pvar.ulVal);	break;
			//RAFAEL sprintf(buffer, "%06u",   pvar.ulVal);	break;
		case VT_R4:
			sprintf_s(buffer, 100, "%6.2f", pvar.fltVal);	break;
			//RAFAEL sprintf(buffer, "%06.1f", pvar.fltVal);	break;
		case VT_R8:
			//LUIZ sprintf(buffer, "%lu",   pvar.dblVal);	break;
			sprintf_s(buffer, 100, "%lf",   pvar.dblVal);	break;
			//RAFAEL sprintf(buffer, "%06.1Lf",   pvar.dblVal);	break;
		case VT_BSTR:
			sprintf_s(buffer, 100, "%ls",    pvar.bstrVal);	break;
			//RAFAEL sprintf(buffer, "%6.6s",    pvar.bstrVal);	break;
		default:
			sprintf_s(buffer, 100, "%s", "");
			vReturn = false;
			break;
	}
	return(vReturn);

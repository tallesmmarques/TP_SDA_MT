// TP_SDA.cpp : Este arquivo contém a função 'main'. A execução do programa começa e termina ali.
//
...
int CheckSocketError(int status, HANDLE hOut) {
	...
	if (status == SOCKET_ERROR) {
		...
		if (erro == WSAEWOULDBLOCK) {
			printf("[Err] Timeout na operacao de RECV! errno = %d - reiniciando...\n\n", erro);
			return(-1); // acarreta reinício da espera de mensagens no programa principal
		}
		else if (erro == WSAECONNABORTED) {
			printf("[Err] Conexao abortada pelo cliente TCP - reiniciando...\n\n");
			return(-1); // acarreta reinício da espera de mensagens no programa principal
		}
		else {
			printf("[Err] Erro de conexao! valor = %d\n\n", erro);
		...
		if (ret == WAIT_TIMEOUT) {
			...
			if (s == INVALID_SOCKET) {
				...
				if (status == WSAENETDOWN) {
					SetConsoleTextAttribute(hOut, HLRED);
					printf("[Err] Rede ou servidor de sockets inacessíveis!\n");
				}
				else {
					SetConsoleTextAttribute(hOut, HLRED);
			...
			// Conecção com servidor socket
			if (connect(s, (SOCKADDR*)&servaddr, sizeof(servaddr)) != 0) {
				status = WSAGetLastError();
				if (status != WSAECONNREFUSED) {
					SetConsoleTextAttribute(hOut, HLRED);
					printf("[Err] Erro na conexao com o servidor. Erro = %d\n", status);
					WSACleanup();
					exit(0);
				}
...
int main(int argc, char** argv)
{
	...
	if (hOut == INVALID_HANDLE_VALUE) {
		SetConsoleTextAttribute(hOut, HLRED);
		printf("[Err] Erro ao obter handle para a saída da console\n");
	}
...
DWORD WINAPI ThreadEnviaMsg(LPVOID id) {
		...
		while (TRUE) {
			...
			if (ret == WAIT_OBJECT_0) {
				...
				if ((nseq_e++) != nseq_r) {
					...
					SetConsoleTextAttribute(hOut, HLRED);
					printf("[Err] Recebeu numero de mensagem estranho: %s\n", buf);
					WSACleanup();
					exit(0);
				}
				if (strncmp(&buf[6], "45", 2) != 0) {
					...
					SetConsoleTextAttribute(hOut, HLRED);
					printf("[Err] Recebeu codigo de mensagem errado : % s\n", &buf[6]);
					WSACleanup();
					exit(0);
				}
				...
				sscanf_s(buf, "%5d", &nseq_r);
				if ((nseq_e++) != nseq_r) {
					// Recebeu numero de mensagem desconhecido
					SetConsoleTextAttribute(hOut, HLRED);
					printf("[Err] Recebeu numero de mensagem estranho: %s\n", buf);
					WSACleanup();
					exit(0);
				...
				SetConsoleTextAttribute(hOut, WHITE);
				printf("[End] Tecla de enceramento do programa ativada...\n");
				acao = -1;
				break;
			}
			else {
				SetConsoleTextAttribute(hOut, HLRED);
				printf("[Err] Erro na thread de leitura de dados de processo. Erro: %d\n", GetLastError()); //retorno ruim
			}
		}
	...
	WSACleanup();
	SetConsoleTextAttribute(hOut, WHITE);
	printf("[End] Encerrando o programa ...");
	_endthreadex(0);
	return 0;
}

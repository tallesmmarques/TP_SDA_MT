...
int main(int argc, char** argv)
{
	...
	hThreadEnviaMsg = (HANDLE)_beginthreadex(
		NULL,
		0,
		(CAST_FUNCTION)thread_envia_msg,	// casting necessário
		(LPVOID) NULL,
		0,
	...
	hThreadOPC = (HANDLE)_beginthreadex(
		NULL,
		0,
		(CAST_FUNCTION)thread_opc,	// casting necessário
		(LPVOID) NULL,
		0,
		(CAST_LPDWORD)&dwThreadId					// casting necessário
	);
...
DWORD WINAPI thread_envia_msg(LPVOID id) {
	...
	short int aborta = serverConnect(); // primeira conexao
	while (!aborta) {
		while (TRUE) {
			...
			if (ret == WAIT_OBJECT_0) {
				...
				desconto += GetTickCount64() - atual; //reducao do tempo de espera ate a proxima msg periodica
		...
		if (acao == -2) aborta = serverConnect();
		else break;
	}
